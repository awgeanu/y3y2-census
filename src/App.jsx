...
function FormView({ onCaptainAccess }) {
  ...
  const toggle = useCallback(hero => {
    setSelected(s => {
      ...
      const existing = s.find(h => isDeadpool ? (h.name === hero.name && h.role === hero.role) : h.name === hero.name);
      return existing
        ? s.filter(h => isDeadpool ? !(h.name === hero.name && h.role === hero.role) : h.name !== hero.name)
        : [...s, hero];
    });
  }, []);
  ...
  const submit = async () => {
    ...
    const ok = await saveStore(store);
    setSaving(false);
    if (ok) setStep("done"); else setSaveErr("Save failed — try again.");
  };

  const goBack = (target = "name") => { if (target === "name") { setName(""); setSelected([]); } setStep(target); };
  ...
  if (step === "done") return (
    <div style={{ minHeight: "100vh", background: "#000", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: FONT, padding: 24 }}>
      <div style={{ textAlign: "center", maxWidth: 420 }}>
        ...
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.25)", marginTop: 6, fontStyle: "italic" }}>{funny}</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center", marginTop: 20 }}>
          {selected.map(h => <HeroPortrait key={h.name + h.role} hero={h} size={42} selected />)}
        </div>
        <ShareButton />
        <div style={{ marginTop: 12 }}>
          <button onClick={() => goBack("portal")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", fontSize: 12, cursor: "pointer", fontFamily: FONT }}>← Back to menu</button>
        </div>
      </div>
    </div>
  );
  ...
  if (step === "pick") return (
    <div style={{ height: "100vh", background: "#000", fontFamily: FONT, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,0.85)", backdropFilter: "blur(20px)", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        <button onClick={() => goBack("portal")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.35)", fontSize: 22, cursor: "pointer", fontFamily: FONT, lineHeight: 1 }}>‹</button>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", fontWeight: 500 }}>Picking for</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#fff", lineHeight: 1.1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "40vw" }}>{name}</div>
        </div>
        <span style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}><span style={{ color: "#60a5fa", fontWeight: 700 }}>{selected.length}</span> selected</span>
        <button onClick={submit} disabled={selected.length === 0 || saving}
          style={{ background: selected.length > 0 ? "rgba(255,255,255,0.95)" : "rgba(255,255,255,0.07)", color: selected.length > 0 ? "#000" : "rgba(255,255,255,0.2)", border: "none", borderRadius: 10, padding: "9px 18px", fontSize: 13, fontWeight: 600, cursor: selected.length > 0 ? "pointer" : "default", fontFamily: FONT }}>
          {saving ? "Saving…" : "Submit"}
        </button>
      </div>
      {saveErr && <div style={{ padding: "8px 16px", background: "rgba(239,68,68,0.1)", fontSize: 11, color: "#f87171" }}>{saveErr}</div>}
      <HeroPicker selected={selected} onToggle={toggle} />
    </div>
  );

  // Comp review
  if (step === "comps") return <PlayerCompReview playerName={name} onBack={() => goBack("portal")} />;
    ...
    <div style={{ height: "100vh", background: "#000", fontFamily: FONT, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,0.85)", backdropFilter: "blur(20px)", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>{name ? "Playing as " + name : "Viewing draft"}</div>
        </div>
        {draftActive && <div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 7, height: 7, borderRadius: "50%", background: "#4ade80", boxShadow: "0 0 6px #4ade80" }} /><span style={{ fontSize: 11, color: "#4ade80", fontWeight: 600, fontFamily: FONT }}>Live</span></div>}
      </div>
  ...
  if (step === "calendar") return <PlayerAvailability playerName={name} onBack={() => goBack("portal")} />;

  // Player portal (after selecting name)
  if (step === "portal") {
    const portalOptions = [
      { icon: "⚔️", label: "Hero Pool", desc: submittedNames.includes(name.toLowerCase()) ? "✓ Pool submitted" : "Submit your hero pool", action: () => setStep("pick"), done: submittedNames.includes(name.toLowerCase()) },
      { icon: "📋", label: "Review Comps", desc: "View captain comps & give feedback", action: () => setStep("comps") },
      { icon: "🎯", label: draftActive ? "Live Draft 🔴" : "Draft Board", desc: draftActive ? "Draft is active — join now" : "View the current draft", action: () => setStep("draft"), live: draftActive },
      { icon: "📅", label: "Availability", desc: "Mark the days you can play", action: () => setStep("calendar") },
    ];
    return (
      <div style={{ minHeight: "100vh", background: "#000", fontFamily: FONT, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 28,
        backgroundImage: "radial-gradient(ellipse 80% 50% at 50% -10%, rgba(255,255,255,0.1), transparent)" }}>
        <div style={{ width: "100%", maxWidth: 400 }}>
          ...
          <div style={{ ...glass({ borderRadius: 20, overflow: "hidden", padding: 0 }) }}>
            {portalOptions.map((opt, i) => (
              <button key={opt.label} onClick={opt.action} style={{
                width: "100%", background: opt.live ? "rgba(248,113,113,0.06)" : "transparent",
                border: "none", borderBottom: i < portalOptions.length - 1 ? "1px solid rgba(255,255,255,0.06)" : "none",
                padding: "16px 20px", display: "flex", alignItems: "center", gap: 14,
                cursor: "pointer", fontFamily: FONT, textAlign: "left", transition: "background .12s",
              }}
              onMouseEnter={e => { e.currentTarget.style.background = opt.live ? "rgba(248,113,113,0.12)" : "rgba(255,255,255,0.05)"; }}
              onMouseLeave={e => { e.currentTarget.style.background = opt.live ? "rgba(248,113,113,0.06)" : "transparent"; }}>
                <div style={{ width: 42, height: 42, borderRadius: 12, background: opt.live ? "rgba(248,113,113,0.15)" : "rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>{opt.icon}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 15, fontWeight: 600, color: opt.live ? "#f87171" : "#fff" }}>{opt.label}</div>
                  <div style={{ fontSize: 12, color: opt.done ? "#4ade80" : "rgba(255,255,255,0.3)", marginTop: 2 }}>{opt.desc}</div>
                </div>
          ...
          <div style={{ marginTop: 20, textAlign: "center" }}>
            <button onClick={() => goBack("name")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.2)", fontSize: 12, cursor: "pointer", fontFamily: FONT }}>← Not you?</button>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{ minHeight: "100vh", background: "#000", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: FONT, padding: 28,
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.18em", color: "rgba(255,255,255,0.3)", marginBottom: 10, textTransform: "uppercase" }}>Y3Y2</div>
          <div style={{ fontSize: 44, fontWeight: 700, letterSpacing: "-0.03em", color: "#fff", lineHeight: 1 }}>Hero Pool</div>
          <div style={{ fontSize: 14, color: "rgba(255,255,255,0.35)", marginTop: 10 }}>Who are you?</div>
        </div>
        ...
        {/* Browse section — no name needed */}
        <div style={{ marginTop: 28 }}>
          <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.12em", color: "rgba(255,255,255,0.2)", textTransform: "uppercase", marginBottom: 10, textAlign: "center" }}>Browse without selecting</div>
          <div style={{ display: "flex", gap: 8 }}>
            {[
              { icon: "📋", label: "Comps", step: "comps" },
              { icon: "🎯", label: "Draft", step: "draft" },
              { icon: "📅", label: "Calendar", step: "calendar" },
            ].map(({ icon, label, step: s }) => (
              <button key={s} onClick={() => { setStep("name"); setPendingStep(s); }} style={{
                flex: 1, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)",
                borderRadius: 12, padding: "14px 8px", cursor: "pointer", fontFamily: FONT,
                display: "flex", flexDirection: "column", alignItems: "center", gap: 6, transition: "background .12s",
              }}
