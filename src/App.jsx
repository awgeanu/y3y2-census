...
function PlayerAvailability({ playerName, onBack }) {
  ...
  const toggle = (d) => {
    const t = new Date(); const today = [t.getFullYear(),String(t.getMonth()+1).padStart(2,'0'),String(t.getDate()).padStart(2,'0')].join('-');
    if (d < today) return; // can't mark past days
    setMarked(m => m.includes(d) ? m.filter(x => x !== d) : [...m, d]);
  };
  ...
  const save = async () => {
    ...
    // Refresh allAvail
    const rows = await loadAllAvailability();
    setAllAvail(rows);
    setSaving(false); setSaved(true);
    setTimeout(() => setSaved(false), 2500);
  };

  // Build a proper calendar grid: start from Sunday of current week
  ...
  startSunday.setDate(today.getDate() - today.getDay()); // go back to Sunday

  // Build 5 weeks = 35 cells using local dates
  const cells = [];
  for (let i = 0; i < 35; i++) {
  ...
  const weeks = [];
  for (let i = 0; i < 35; i += 7) weeks.push(cells.slice(i, i+7));

  // Count teammates per day
  const countMap = {};
  const nameMap = {}; // day -> [names]
  allAvail.filter(r => r.player_name !== playerName).forEach(r => {
    (r.available_dates || []).forEach(d => {
      countMap[d] = (countMap[d] || 0) + 1;
      if (!nameMap[d]) nameMap[d] = [];
      nameMap[d].push(r.player_name);
    });
  });

  // Activity feed — sorted by updated_at
  const activity = [...allAvail].sort((a,b) => new Date(b.updated_at) - new Date(a.updated_at)).slice(0, 8);

  // Get current month label for the header row
    ...
    <div style={{ height: '100vh', background: '#000', fontFamily: FONT, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(20px)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)' }}>Tap days you can play · CT · ET+1 · PT−2</div>
        </div>
        <button onClick={save} disabled={saving}
          style={{ background: saved ? 'rgba(74,222,128,0.15)' : 'rgba(255,255,255,0.92)', border: saved ? '1px solid rgba(74,222,128,0.4)' : 'none', borderRadius: 10, padding: '8px 18px', color: saved ? '#4ade80' : '#000', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: FONT, transition: 'all .2s', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 14px 24px' }}>
        {/* Calendar */}
        <div style={{ ...glass({ borderRadius: 16, padding: '14px 12px', marginBottom: 16 }) }}>
          {/* Day headers */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 3, marginBottom: 6 }}>
            {['S','M','T','W','T','F','S'].map((d,i) => (
              <div key={i} style={{ textAlign: 'center', fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.25)', padding: '4px 0' }}>{d}</div>
            ))}
          </div>
          {/* Weeks */}
          {weeks.map((week, wi) => {
            // Show month label when month changes
            const firstOfWeek = new Date(week[0] + 'T12:00:00');
            const showMonth = wi === 0 || firstOfWeek.getDate() <= 7;
            return (
              <div key={wi}>
                {showMonth && (
                  <div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.2)', letterSpacing: '0.08em', textTransform: 'uppercase', padding: '6px 2px 4px', marginTop: wi > 0 ? 4 : 0 }}>
                    {MONTH_LABELS[firstOfWeek.getMonth()]} {firstOfWeek.getFullYear()}
                  </div>
                )}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 3, marginBottom: 3 }}>
                  {week.map(day => {
                    const d = new Date(day + 'T12:00:00');
                    const cellLocal = [d.getFullYear(), String(d.getMonth()+1).padStart(2,'0'), String(d.getDate()).padStart(2,'0')].join('-');
                    const isPast = cellLocal < todayStr;
                    const isToday = cellLocal === todayStr;
                    const isMine = marked.includes(day);
                    const teammates = nameMap[day] || [];
                    const teamCount = teammates.length;
                    return (
                      <button key={day} onClick={() => toggle(day)} disabled={isPast}
                        style={{
                          background: isMine ? 'rgba(74,222,128,0.2)' : isToday ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.02)',
                          border: isMine ? '1px solid rgba(74,222,128,0.5)' : isToday ? '1px solid rgba(255,255,255,0.25)' : '1px solid rgba(255,255,255,0.04)',
                          borderRadius: 8, padding: '6px 2px', cursor: isPast ? 'default' : 'pointer',
                          fontFamily: FONT, textAlign: 'center', transition: 'all .1s',
                          opacity: isPast ? 0.25 : 1, minHeight: 44,
                        }}>
                        <div style={{ fontSize: 14, fontWeight: isToday ? 700 : 500, color: isMine ? '#4ade80' : isToday ? '#fff' : 'rgba(255,255,255,0.6)', lineHeight: 1 }}>{d.getDate()}</div>
                        {teamCount > 0 && (
                          <div style={{ marginTop: 3, display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: 2 }}>
                            {Array.from({ length: Math.min(teamCount, 3) }).map((_,i) => (
                              <div key={i} style={{ width: 4, height: 4, borderRadius: '50%', background: '#60a5fa', opacity: 0.7 }} />
                            ))}
                            {teamCount > 3 && <div style={{ fontSize: 7, color: '#60a5fa', fontWeight: 700, lineHeight: 1 }}>+{teamCount-3}</div>}
                          </div>
                        )}
                      </button>
                    );
          ...
          {/* Legend */}
          <div style={{ display: 'flex', gap: 14, marginTop: 10, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: 'rgba(255,255,255,0.3)' }}>
              <div style={{ width: 10, height: 10, borderRadius: 3, background: 'rgba(74,222,128,0.25)', border: '1px solid rgba(74,222,128,0.5)' }} /> You
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: 'rgba(255,255,255,0.3)' }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#60a5fa', opacity: 0.7 }} /> Teammates available
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: 'rgba(255,255,255,0.3)' }}>
              <div style={{ width: 10, height: 10, borderRadius: 3, border: '1px solid rgba(255,255,255,0.25)' }} /> Today
            </div>
          </div>
        </div>
        ...
        {/* Activity feed */}
        <div>
          <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.1em', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: 10 }}>Recent Activity</div>
          {activity.length === 0 ? (
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.15)', textAlign: 'center', padding: '16px 0' }}>No activity yet</div>
          ) : activity.map((a, i) => {
            const dates = (a.available_dates || []).filter(d => d >= todayStr).sort();
              ...
              <div key={i} style={{ ...glass({ borderRadius: 10, padding: '10px 14px', marginBottom: 6 }), display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginTop: 1 }}>
                    {dates.length > 0 ? `${dates.length} day${dates.length !== 1 ? 's' : ''} available${nextFormatted ? ' · next: ' + nextFormatted : ''}` : 'No upcoming days marked'}
                  </div>
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)', flexShrink: 0 }}>{relativeTime(a.updated_at)}</div>
              </div>
            );
          })}
        </div>
...
function DraftDashboard({ roster, responses, playerName, isCaptain }) {
  ...
  const poll = async () => {
    ...
    if (d !== null) {
      ...
      // Captain owns their draft — only endSession() can clear it
      // Players rely on polling so they see null = no draft
      if (!isCaptain) {
        setDraft(null);
        setLoading(false);
      } else {
        // For captain: only clear if loading (first load), never mid-session
        setDraft(prev => {
          if (prev === null && loading) return null; // first load, nothing there
          return prev; // keep whatever captain has locally
        });
        setLoading(false);
      }
    }
  ...
  const startDraft = async (mode) => {
    ...
    await new Promise(r => setTimeout(r, 1200)); // let Supabase settle
    nullCountRef.current = 0; // reset null counter after successful save
  ...
  const endSession = async () => {
    savingRef.current = true;
    setDraft(null); // clear locally immediately
    await clearDraft();
    savingRef.current = false;
  };
...
function CalendarDashboard() {
  ...
  // Local today string (no UTC shift)
  const todayLocal = new Date();
  const todayStr = [todayLocal.getFullYear(), String(todayLocal.getMonth()+1).padStart(2,'0'), String(todayLocal.getDate()).padStart(2,'0')].join('-');

  // Build week-aligned grid: start from Sunday of current week
  const startSunday = new Date(todayLocal);
  startSunday.setHours(0,0,0,0);
  startSunday.setDate(todayLocal.getDate() - todayLocal.getDay());
  const cells = [];
  for (let i = 0; i < 35; i++) {
    const d = new Date(startSunday);
    d.setDate(startSunday.getDate() + i);
    cells.push([d.getFullYear(), String(d.getMonth()+1).padStart(2,'0'), String(d.getDate()).padStart(2,'0')].join('-'));
  }
  const weeks = [];
  for (let i = 0; i < 35; i += 7) weeks.push(cells.slice(i, i+7));
  ...
  // Activity feed sorted by recency
  const activityFeed = [...avail].sort((a,b) => new Date(b.updated_at)-new Date(a.updated_at)).slice(0, 12).map(r => ({
